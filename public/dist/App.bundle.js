...
function cartUpdate(e) {
  ...
  _axios2.default.post(this.action).then(function (res) {
    _this.buy.classList.toggle('disabled');
    _this.children.preditable.classList.add('ui', 'inverted', 'pink');
    (0, _bling.$)('.cart-length').textContent = res.data.content.length;
  }).catch(console.error);
}

exports.default = cartUpdate;
...
function followUser(e) {
  ...
  e.preventDefault();
  _axios2.default.post(this.action).then(function (res) {
    if (_this.follow.value === 'Following') {
      _this.follow.value = 'Follow';
    } else {
      _this.follow.value = 'Following';
    }
    _this.follow.classList.toggle('following');
    (0, _bling.$)('.followers-count').textContent = '' + res.data.followers.length;
  }).catch(console.error);
}
...
function removeCart(e) {
  ...
  _axios2.default.post(this.action).then(function (res) {
    (0, _bling.$)('.cart-length').textContent = res.data.content.length;
    (0, _bling.$)('.total-price').textContent = 'Total: KES ' + res.data.totalPrice;
    _this.remove(_this);
  }).catch(console.error);
}
...
function typeAhead(search) {
  ...
  searchInput.on('keyup', function (e) {
    ...
    var activeClass = 'grey';
    var current = search.querySelector('.' + activeClass);
    var items = search.querySelectorAll('.result');
    var next = void 0;
...
var _removecart = __webpack_require__(14);
...
var _addprediction = __webpack_require__(11);
